...
Docker Compose bind-mounts **`./data/geomagic-3dsystems`** onto **`/usr/share/3DSystems`** so pairing persists across containers and stays visible no disco do projeto. If you use an old image, rebuild.
...
| Persistence | N/A (bare metal) | **`./data/geomagic-3dsystems`** bind-mounted to **`/usr/share/3DSystems`** (gitignored). |
